...
<%
/**
 * $Id: tiny_mce_gzip.jsp 159 2006-12-21 15:03:35Z spocke $
 *
 * @author Moxiecode
 ...
 */
	...
	boolean diskCache, supportsGzip, isJS, compress;
...
//	response.setContentType("application/x-javascript");
	...
	if (diskCache) {
		...
		if (compress)
			cacheFile = cachePath + File.separatorChar + "tiny_mce_" + cacheKey + (compress?".gz":".js");
		else
...
%><%!
	...
	public String mapPath(HttpServletRequest request, String path) {
		String absPath =  getServletConfig().getServletContext().getRealPath(request.getRequestURI().replaceFirst(getServletConfig().getServletContext().getServletContextName(),""));
        absPath = absPath.substring(0, absPath.lastIndexOf(File.separatorChar) + 1);
		String re=absPath + path.replace('/', File.separatorChar);
        return re;
	}
